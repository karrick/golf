...
func init() {
	...
	if location := os.Getenv("GOLF_DEBUG"); location != "" {
		...
		default:
			...
			if err != nil {
				...
				_, _ = fmt.Fprintf(os.Stderr, "%s GOLF_DEBUG: cannot open file: %q: %s\n", now, location, err)
				fh = os.Stderr
			}
		}
		...
		debug = func(f string, a ...interface{}) {
			...
			if f != "" && f[len(f)-1] == '\n' {
				_, _ = fmt.Fprintf(fh, "%s GOLF_DEBUG: %s", now, content)
			} else {
				_, _ = fmt.Fprintf(fh, "%s GOLF_DEBUG: %s\n", now, content)
			}
		}
